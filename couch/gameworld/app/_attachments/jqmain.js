...
///////////////////////////////////////Week 4 files////////////////////////////////////////////
//couch new member JSON data page
$('#newMemPage').on('pageshow', function() {
    console.log("couch data page ready to create new member list list!");
...
//Page-ready for favCard Page
$('#favCardPage').on('pageinit', function(){
    console.log("Fav Card  page loaded!");
});


//saveData to couch function
...
var saveData= function(data){
        ...
        var doc            ={};
            doc._id     = "mem" + $('#lname').val() + id;
            doc.fname        =$('#fname').val();
            doc.lname        =$('#lname').val();
            doc.pword        =$('#pword').val();
            doc.cpword       =$('#cpword').val();
            doc.email        =$('#email').val();
            doc.friends      =$('#quantity').val();
            doc.day          =$('#day').val();
            doc.month        =$('#month').val();        
            doc.year         =$('#year').val();        
            doc.comments     =$('#comments').val();

                $.couch.db("gameworld").saveDoc(doc, {
                    success: function(data) {
                        alert("data saved!");
                    },
                    error: function(status) {
...
$('#newMemPage').on('pageinit', function(){
    console.log("new Mem page loaded!");
});
